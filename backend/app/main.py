...
async def _process_scan_job(job_id: str):
    ...
    try:
        ...
        try:
            ...
            if severity in ("high", "critical") and p.exists():
                try:
                    ...
                    if deletion_result.get("success"):
                        print(f"[Anomaly Auto-Delete] ✓ Deleted {deletion_result.get('item_type', 'item')} immediately: {p.name} (method: {deletion_result.get('method')})")
                        # Update threat description to indicate auto-deletion
                        from sqlmodel import Session
                        with Session(DB.engine) as s:
                            threat_obj = s.get(Threat, threat_id)
                            if threat_obj:
                                threat_obj.action = "deleted"
                                threat_obj.description = f"{threat_obj.description} [Auto-deleted on detection]"
                                s.add(threat_obj)
                                s.commit()
                    else:
                        print(f"[Anomaly Auto-Delete] ✗ Failed to delete: {p.name} - {deletion_result.get('error', 'Unknown error')}")
                except Exception as del_e:
                    print(f"[Anomaly Auto-Delete] Error during auto-deletion: {del_e}")
        except Exception as e:
